...
while True:
        ...
        for f in flist:
            ...
            frames0 = np.arange(0,1000)
            frames1 = np.arange(1000,2000)
            frames2 = np.arange(2000,3000)

            sel = trajs['frame'].isin(frames0)
            currtrajs = trajs[sel]
            for tnum in currtrajs['trajectory'].unique():
                currtraj = currtrajs[currtrajs['trajectory']==tnum]
                ax[0,1].plot(currtraj['x'],currtraj['y'],'-',color='blue')
                ax[0,1].plot(currtraj['x'].iloc[0],currtraj['y'].iloc[0],'.',color='darkblue')
                
            sel = trajs['frame'].isin(frames1)
            currtrajs = trajs[sel]
            for tnum in currtrajs['trajectory'].unique():
                currtraj = currtrajs[currtrajs['trajectory']==tnum]
                ax[0,1].plot(currtraj['x'],currtraj['y'],'-',color='red')
                ax[0,1].plot(currtraj['x'].iloc[0],currtraj['y'].iloc[0],'.',color='darkred')
            ax[0,1].axis('off');
            
            sel = trajs['frame'].isin(frames2)
            currtrajs = trajs[sel]
            for tnum in currtrajs['trajectory'].unique():
                currtraj = currtrajs[currtrajs['trajectory']==tnum]
            ...
            traj0 = trajs[trajs['frame'].isin(frames0)]
            d0 = getalldisp(traj0)
            traj1 = trajs[trajs['frame'].isin(frames1)]
            d1 = getalldisp(traj1)
            traj2 = trajs[trajs['frame'].isin(frames2)]
            d2 = getalldisp(traj2)

            h0,g0 = np.histogram(d0*umperpx,bins=50);
            h0 = h0/h0.sum()
            h1,g1 = np.histogram(d1*umperpx,bins=50);
            h1 = h1/h1.sum()
            h2,g2 = np.histogram(d2*umperpx,bins=50);
            h2 = h2/h2.sum()
            
            ax[1,0].plot((g0[1:]+g0[:-1])/2,h0,color='blue',label='Phase 0')
            ax[1,0].plot((g1[1:]+g1[:-1])/2,h1,color='red',label='Phase 1')
            ax[1,0].plot((g2[1:]+g2[:-1])/2,h2,color='violet',label='Phase 2')
